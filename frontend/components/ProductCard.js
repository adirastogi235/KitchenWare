...
export default function ProductCard({ product }) {
  ...
  return (
    <Link href={`/products/${product.id}`} className="group block">
      <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] overflow-hidden transition-all duration-500 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-black/[0.06] hover:border-[var(--color-primary)]/30">
        <div className="relative aspect-square overflow-hidden bg-[var(--color-surface-2)]">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {product.featured && (
              <span className="px-2.5 py-1 bg-[var(--color-primary)] text-[#1a1410] text-xs font-bold rounded-full shadow-lg shadow-yellow-400/25">
                Featured
              </span>
            )}
        ...
        <div className="p-4">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-medium text-[var(--color-primary)] bg-[var(--color-primary)]/10 px-2.5 py-0.5 rounded-full">
              {product.category}
            </span>
            {product.brand && (
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {qty === 0 ? (
              <button
                ...
                className="px-4 py-2 rounded-full bg-[var(--color-primary)] text-[#1a1410] text-sm font-medium hover:shadow-lg hover:shadow-yellow-400/30 transition-all duration-300 hover:-translate-y-0.5 disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none active:scale-95"
              >
              ...
              <div
                onClick={stopLink}
                className="flex items-center gap-1 rounded-full bg-[var(--color-primary)] p-1 shadow-lg shadow-yellow-400/30"
              >
                <button
                  onClick={handleDecrement}
                  disabled={busy}
                  aria-label="Decrease quantity"
                  className="w-7 h-7 flex items-center justify-center rounded-full bg-black/20 hover:bg-black/30 text-[#1a1410] font-bold text-lg leading-none transition-colors disabled:opacity-50 active:scale-90"
                >
                  −
                </button>
                <span className="min-w-[1.5rem] text-center text-[#1a1410] text-sm font-bold tabular-nums">
                  {qty}
                </span>
                <button
